...
import { IabititiStat, IgraphElement, IgraphLegend, Istat, IvoleyStats } from "../interfaces"
...
export default function PerformanceDonutGraph({ stats }: {stats: IvoleyStats | undefined}){
  ...
  const graphs = () => {
    ...
    if (!graphList.length) return <p>No Statistics available</p>
    return (
  ...
  const legend = () => {
    ...
    return (
      <div className="legend">
        <div className="stat">
          <label>Prev<input type="checkbox" onChange={ () => setIncludePrevious(!includePrevious) }/></label>
          <p>Good:</p>
